...
constraint fk_commande foreign key (id_client) references clients (id_client)
);
...
constraint pk_detail_commande primary key(is_deleted_DC),
...
constraint fk_paiement foreign key (id_comm) references commande (id_comm)
);
